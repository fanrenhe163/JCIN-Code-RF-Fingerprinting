...
for snr in snr_values:
    ...
    print(f"方法1：提取15个特征并使用神经网络分类")
    features_file = os.path.join(
        npy_dir, f"features_method1_snr_{snr}_devices_{len(device_list)}.npy"
    )
    labels_features_file = os.path.join(
        npy_dir, f"labels_method1_snr_{snr}_devices_{len(device_list)}.npy"
    )
    if os.path.exists(features_file) and os.path.exists(labels_features_file):
        print("特征已提取，正在从文件加载特征和标签...")
        data_method1 = np.load(features_file)
        labels_method1 = np.load(labels_features_file)
    else:
        print("开始特征提取...")
        data = []
        ...
        print("特征提取完成。")
        data_method1 = np.array(data)
        labels_method1 = selected_labels
        np.save(features_file, data_method1)
        np.save(labels_features_file, labels_method1)
        print(f"特征已保存到 {features_file}")
    k = 5
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=16)
    input_size = data_method1.shape[1]
    num_classes = num_selected_devices
    all_train_acc = []
    all_test_acc = []
    fold = 0
    for train_index, test_index in skf.split(data_method1, labels_method1):
        fold += 1
        print(f"折数 {fold}:")
        model_save_path_fold = os.path.join(
            model_dir, f"method1_model_snr_{snr}_fold_{fold}.pt"
        )
        if os.path.exists(model_save_path_fold):
            ...
            model.eval()
            with torch.no_grad():
                X_test_fold = data_method1[test_index]
                Y_test_fold = labels_method1[test_index]
                scaler = StandardScaler()
                X_train_fold = data_method1[train_index]
            ...
            continue
        X_train_fold, X_test_fold = data_method1[train_index], data_method1[test_index]
        Y_train_fold, Y_test_fold = (
            labels_method1[train_index],
            labels_method1[test_index],
        )
        scaler = StandardScaler()
    ...
    print(
        f"方法1 SNR = {snr} dB, 平均训练准确率: {avg_train_acc:.4f}, 平均测试准确率: {avg_test_acc:.4f}"
    )
    method1_results[snr] = avg_test_acc
    print(f"方法2和方法3：小波变换 + ReliefF 特征选择 + PCA + 分类器")
    wavelet_features_file = os.path.join(
        npy_dir, f"wavelet_features_snr_{snr}_devices_{len(device_list)}.npy"
    )
    labels_wavelet_file = os.path.join(
        npy_dir, f"labels_wavelet_snr_{snr}_devices_{len(device_list)}.npy"
    )
    if os.path.exists(wavelet_features_file) and os.path.exists(labels_wavelet_file):
        print("小波特征已提取，正在从文件加载特征和标签...")
        data_wavelet = np.load(wavelet_features_file)
        labels_wavelet = np.load(labels_wavelet_file)
    else:
        print("开始小波特征提取...")
        data = []
        for signal in selected_signals:
            coeffs = pywt.wavedec(signal, "haar", level=10)
            coeffs_flat = np.concatenate(coeffs)
            data.append(np.abs(coeffs_flat))
        data_wavelet = np.array(data)
        labels_wavelet = selected_labels
        np.save(wavelet_features_file, data_wavelet)
        np.save(labels_wavelet_file, labels_wavelet)
        print(f"小波特征已保存到 {wavelet_features_file}")
    X_train, X_test, y_train, y_test = train_test_split(
        data_wavelet,
        labels_wavelet,
        test_size=0.3,
        random_state=16,
        stratify=labels_wavelet,
    )
    num_features_relieff = 100
    relieff = ReliefF(n_neighbors=100, n_features_to_select=num_features_relieff)
    X_train_selected = relieff.fit_transform(X_train, y_train)
    X_test_selected = relieff.transform(X_test)
    pca = PCA(n_components=0.85)
    X_train_pca = pca.fit_transform(X_train_selected)
    X_test_pca = pca.transform(X_test_selected)
    svm_model_save_path = os.path.join(model_dir, f"method2_svm_model_snr_{snr}.joblib")
    if os.path.exists(svm_model_save_path):
        print(f"找到已保存的 SVM 模型，正在加载 {svm_model_save_path}")
        svm_model = joblib.load(svm_model_save_path)
    else:
        svm_model = SVC()
        svm_model.fit(X_train_pca, y_train)
        joblib.dump(svm_model, svm_model_save_path)
        print(f"SVM 模型已保存到 {svm_model_save_path}")
    y_pred_svm = svm_model.predict(X_test_pca)
    test_acc_svm = accuracy_score(y_test, y_pred_svm)
    print(f"方法2 SNR = {snr} dB, 测试准确率: {test_acc_svm:.4f}")
    method2_results[snr] = test_acc_svm
    rf_model_save_path = os.path.join(model_dir, f"method3_rf_model_snr_{snr}.joblib")
    if os.path.exists(rf_model_save_path):
        print(f"找到已保存的随机森林模型，正在加载 {rf_model_save_path}")
        rf_model = joblib.load(rf_model_save_path)
    else:
        rf_model = RandomForestClassifier(n_estimators=100, random_state=42)
        rf_model.fit(X_train_pca, y_train)
        joblib.dump(rf_model, rf_model_save_path)
        print(f"随机森林模型已保存到 {rf_model_save_path}")
    y_pred_rf = rf_model.predict(X_test_pca)
    test_acc_rf = accuracy_score(y_test, y_pred_rf)
    print(f"方法3 SNR = {snr} dB, 测试准确率: {test_acc_rf:.4f}")
    method3_results[snr] = test_acc_rf
results_file_method1 = os.path.join(
    npy_dir, f"method1_results_devices_{len(device_list)}.pkl"
)
with open(results_file_method1, "wb") as f:
    pickle.dump(method1_results, f)
results_file_method2 = os.path.join(
    npy_dir, f"method2_results_devices_{len(device_list)}.pkl"
)
with open(results_file_method2, "wb") as f:
    pickle.dump(method2_results, f)
results_file_method3 = os.path.join(
    npy_dir, f"method3_results_devices_{len(device_list)}.pkl"
)
with open(results_file_method3, "wb") as f:
    pickle.dump(method3_results, f)
